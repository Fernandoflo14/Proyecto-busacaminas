...
public class BuscaminasGUI extends JFrame {
    ...
    // ===== TEMA CELESTE =====
    private static final Color APP_BG = new Color(233, 245, 255);
    private static final Color TOP_BG = new Color(210, 236, 255);
    private static final Color BOARD_BG = new Color(200, 225, 245);

    private static final Color CELL_CLOSED_BG = new Color(245, 250, 255);
    private static final Color CELL_HOVER_BG  = new Color(230, 244, 255);
    private static final Color CELL_OPEN_BG   = new Color(214, 232, 245);

    private static final Color CELL_BORDER    = new Color(170, 200, 220);
    ...
    private void cambiarNivel(Nivel nivel) {
        ...
        tablero = new JPanel(new GridLayout(SIZE, SIZE, 6, 6));
    ...
    private void handleLeftClick(int i, int j) {
        ...
        // No abrir si está con bandera
        if (FLAG_TEXT.equals(btn.getText())) return;
        ...
        if (minas[i][j]) {
            ...
            String nombre = JOptionPane.showInputDialog(
                    this,
                    "Perdiste. Ingresa tu nombre para el ranking:",
                    "Jugador"
            );
            ...
            JOptionPane.showMessageDialog(
                    this,
                    "¡Perdiste! Has explotado una mina.",
                    "Juego terminado",
            ...
            // ===== AQUÍ ESTÁ EL CAMBIO: abrir vacíos usando PILA =====
            if (conteo == 0) {
                abrirCeldasVaciasConPila(i, j);
            }
        }
    }
    ...
    private void checkWin() {
        ...
        if (!gameOver && (ganoPorAbrir || ganoPorBanderas)) {
            ...
            String nombre = JOptionPane.showInputDialog(
                    this,
                    "¡Ganaste! Ingresa tu nombre para el ranking:",
                    "Jugador"
            );
    ...
    private void applyNumberColor(JButton btn, int n) {
        switch (n) {
            case 1: btn.setForeground(new Color(30, 120, 200)); break;
            case 2: btn.setForeground(new Color(30, 160, 90)); break;
            case 3: btn.setForeground(new Color(220, 120, 40)); break;
            case 4: btn.setForeground(new Color(60, 90, 220)); break;
            case 5: btn.setForeground(new Color(160, 70, 200)); break;
    ...
    // ===== NUEVO: flood fill con PILA (estructura #3) =====
    private void abrirCeldasVaciasConPila(int i, int j) {
        Pila<Cell> pila = new Pila<>();
        pila.push(new Cell(i, j));
            ...
            for (int di = -1; di <= 1; di++) {
                for (int dj = -1; dj <= 1; dj++) {
                    ...
                    if (ni >= 0 && nj >= 0 && ni < SIZE && nj < SIZE) {
                        ...
                        if (!nb.isEnabled()) continue;
                        if (FLAG_TEXT.equals(nb.getText())) continue;
